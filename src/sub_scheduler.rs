            ...
            match self.jobs_queue.pop() {
                Some(mut next_allocation) => {
                    if next_allocation.job.res <= self.resources.range_size() as i32 {
                        ...
                        interval = Interval::new(self.resources.get_inf(), self.resources.get_inf() + nb_resources - 1);
                    } else {
                        let ressources_to_complete = cmp::min(self.resources.range_size(),
                                                              next_allocation
                                                                  .nb_of_res_to_complete());
                        interval = Interval::new(self.resources.get_inf(), self.resources.get_inf() + ressources_to_complete - 1);
                    }
                    // We upate the allocation with the resources
                    // allocated by the scheduler
                    Rc::make_mut(&mut next_allocation)
                        .add_resources(interval.to_interval_set());
